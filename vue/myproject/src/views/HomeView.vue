  ...
  methods: {
    handleLogin() {
      ...
      const data = {
        ...
        const url = `${this.$root.$baseUrl}/users/login`;
        // const url = `${this.$root.$baseUrl}/users/login?id=${data.id}&password=${data.password}`;
    ...
    handleRegister() {
      ...
      const data = {
        ...
        const url = `${this.$root.$baseUrl}/users/register`;
        this.axios.post(url, data,{ withCredentials: true })
              .then(response => {
                const data = response.data; 
                const code = data.code; 
